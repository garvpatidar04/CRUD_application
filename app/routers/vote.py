...
def vote(vote: schemas.Vote, db: Session = Depends(database.get_db), current_user:int = Depends(oauth2.get_current_user)):
    ...
    if vote.dir==1:
        if vote_exist:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=f"You have already voted")
        ...
        if not vote_exist:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"you did not vote this post")
